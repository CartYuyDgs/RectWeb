...
import (
	...
	"mecas/cmd/node_iface_agent/message"
	"mecas/cmd/node_iface_agent/onandoff"
	"net/http"
	"net/url"
	"time"
...
func init() {
	...
	HostSend.Hostname = conf.HostName
...
func ConnectionServer() (conn *chans.Connection) {
	u := url.URL{
		Scheme: "ws",
		Host:   Host,
		Path:   Path,
	...
	// defer conn.Close()
	conn = chans.CreateConnection(WsConn)
	conn.IsClosed = false
	return
}
...
func sendStatistics(conn *chans.Connection) {
...
// 接收服务端发回的指令，并执行指令
func receiveCommand(conn *chans.Connection) {
	var (
		err error
		msg []byte
	)
...
func ONOrOFFDiscover(conn *chans.Connection,name string, status int) {

...
func (o *OnOrOffDisc) toStart(conn *Connection) error {
	o.ctx, o.cancle = CreateCTX()
	stat, err := NicStatusUp(o.NicName)
	if err != nil || stat != "up" {
...
func (o *OnOrOffDisc) sendOnOrOffDiscoverInfo(conn *Connection) {
	ticker := time.NewTicker(nicFrequency)
	defer ticker.Stop()
	for {
		select {
		case <-ticker.C:
			stat := LinkDetectedget(o.NicName)
